<template>
  <demo-section>
    <demo-block :title="$t('basicUsage')">
      ...
      <ii-actionsheet v-model="show1" :actions="actions" @select="onSelect" />
    </demo-block>
    ...
    <demo-block :title="$t('title3')">
      <ii-button @click="show3 = true">{{ $t('button3') }}</ii-button>
      <ii-actionsheet v-model="show3" :title="$t('title')">
        <p>{{ $t('content') }}</p>
      </ii-actionsheet>
    </demo-block>

    <demo-block title="弹出slot列表">
      <ii-button @click="show4 = true">{{ $t('button3') }}</ii-button>
      <ii-actionsheet v-model="show4" :cancel-text="'取消'">
        <li>{{ $t('content') }}</li>
      </ii-actionsheet>
    </demo-block>
  </demo-section>
