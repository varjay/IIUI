...
const createComponent = () => {
  ...
  const button = wrapper.find('.ii-button');
  const field = wrapper.findAll('.ii-field__control');
...
test('on change detail', () => {
  const wrapper = mount(AddressEdit);
  const field = wrapper.findAll('.ii-field__control').at(3);
...
test('show search result', async() => {
  ...
  const items = wrapper.findAll('.ii-icon-location');
...
test('delete address', async() => {
  ...
  const deleteButton = wrapper.findAll('.ii-button').at(1);
  deleteButton.trigger('click');
  document.querySelector('.ii-dialog__cancel').click();
  deleteButton.trigger('click');
  document.querySelector('.ii-dialog__confirm').click();
