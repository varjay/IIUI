...
test('get container with parent', () => {
  ...
  const popup = wrapper.find('.ii-popup').element;
...
test('render overlay', () => {
  ...
  expect(div.querySelector('.ii-modal')).toBeFalsy();
  wrapper.vm.overlay = true;
  expect(div.querySelector('.ii-modal')).toBeTruthy();
});
...
test('close on click modal', () => {
  ...
  const modal = div.querySelector('.ii-modal');
