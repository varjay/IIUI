...
以下是关于向 IIUI 提交代码的指南。在向 IIUI 提交 Issue 或者 PR 之前，请先花几分钟时间阅读以下文字。
...
- 组件样式代码位于 packages/iiui-css/src 下，iiui-css 也会在发布时单独发包
...
IIUI 中的组件会通过 `create` 方法统一创建，`create` 方法会在选项文件中注入基本的依赖和方法。
...
组件样式使用 `precss 2.0` 作为预处理器，具体语法请参考 [precss 文档](https://github.com/jonathantneal/precss).
