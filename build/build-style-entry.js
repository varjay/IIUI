...
components.forEach(component => {
  ...
  const esContent = deps.map(dep => `import '../../ii-css/${dep}.css';`).join('\n');
  const libContent = deps.map(dep => `require('../../ii-css/${dep}.css');`).join('\n');
...
function checkComponentHasStyle(component) {
  return fs.existsSync(
    path.join(__dirname, `../es/ii-css/`, `${component}.css`)
  );
}
